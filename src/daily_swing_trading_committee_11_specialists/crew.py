...
# [설정] 무료 Groq 모델 정의 (가장 성능이 좋은 Llama 3.1 70B 사용)
GROQ_LLM = LLM(
...
class DailySwingTradingCommittee11SpecialistsCrew:
    ...
    def send_telegram_msg(self, result):
        ...
        if token and chat_id:
            ...
            text = f"🚀 [AI 투자 위원회 결론]\n\n{str(result)}"
            # 텔레그램 메시지 제한(4096자)을 고려하여 자름
            requests.post(url, json={"chat_id": chat_id, "text": text[:4000]})

    # --- 에이전트 정의 (모든 에이전트의 LLM을 GROQ로 변경) ---
    ...
    def senior_investment_committee_moderator(self) -> Agent:
        return Agent(
            ...
            llm=GROQ_LLM,
            verbose=True,
            allow_delegation=True # 의장은 다른 에이전트에게 질문할 수 있게 허용
        )
    ...
    @task
    def daily_fundamental_swing_catalyst_analysis(self)